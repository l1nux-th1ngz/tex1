...
# Download and install JDK packages
...
# Enable and update Bookworm backports
echo 'deb http://deb.debian.org/debian bookworm-backports main contrib non-free-firmware' | tee /etc/apt/sources.list.d/bookworm-backports.list
echo 'deb-src http://deb.debian.org/debian bookworm-backports main contrib non-free-firmware' | tee -a /etc/apt/sources.list.d/bookworm-backports.list
apt update
echo "-------------------------------------------------------------------------------"
echo "---BOOKWORM BACKPORTS ENABLED AND UPDATED---"
echo "-------------------------------------------------------------------------------"

apt-get install -t bookworm-backports linux-image-amd64/bookworm-backports linux-headers-amd64/bookworm-backports firmware-linux
echo ""
echo "Please reboot into your new kernel when able."
...
echo 'slim ALL=(ALL) NOPASSWD:ALL' >> /etc/sudoers

# Switch to the slim user
su - slim
